...
lgb100 = pd.read_csv('../datas/lgb60_month_6_yval_pred')
...
bi = 0
xbs = []
while lgb100.loc[xi]['is_risk']==1:
    xbs.append(lgb100.loc[xi]['rowkey'])
    if lgb100.loc[xi]['rowkey'] in bs:
        bi += 1
    yi2 = lgb100.loc[xi]['y']
    xi += 1
    if xi%10 ==0:
...
xgb100 = pd.read_csv('../datas/lgb60_month_1_ytest_pred')
lgb80 = pd.read_csv('../datas/lgb60_more_ytest_pred')
lgb100 = pd.read_csv('../datas/lgb60_month_6_ytest_pred')
...
xbs = lgb100[lgb100['y']>=yi2]['rowkey'].values.tolist()
print len(set(bs)),len(xbs)
xxi = 0
for xbi in range(len(xbs)):
    if xbs[xbi] in bs:
        xxi += 1
