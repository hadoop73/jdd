...
feas = pd.read_csv('../datas/all3.csv')  # 0.6 filter
print(feas.shape)

df_train = df_train.merge(feas,on='rowkey')

feas = pd.read_csv('../datas/feas_new_more')
feas = feas.drop(['hour','hour_v','weekday'],axis=1)
print(feas.shape)
...
# 构造训练集
df_train = df_train[df_train['time']>='2015-03-01 00:00:00']
...
params = {
    ...
    'bagging_freq': 60,          # bagging 每 5 次进行
    'max_bin':80,              # 特征最大分割
...
print(df_train.shape)
print("top n: ",n)
print(dtest[dtest['y']>=yi].shape[0])
# 预训练证集
