...
xstr = 'lgb60_more31'
feassum = pd.read_csv('../datas/feas_new_sum')

feas = pd.read_csv('../datas/feas_new')
feas = feas.drop('is_risk',axis=1)

feas = feas.merge(feassum,on='rowkey')

feas3 = pd.read_csv('../datas/feas_login_more')
#feas3['login_type3_hour_cnt2'] = feas3['login_type3_hour_cnt']*feas3['trade_hour_cnt']
#feas3 = feas3.drop(['trade_login_hour_cnt'],axis=1)
feas3 = feas3.drop(['time','id'],axis=1)
feas = feas.merge(feas3,on='rowkey')
...
params = {
    ...
    'bagging_fraction': 0.7,    # 随机选择 80% 的数据进行 bagging
