...
feas = pd.read_csv('../datas/feas_login_new')

feas = feas.drop(['id','hour','time'],axis=1)

fea = pd.read_csv('../datas/feas_new')
fea = fea.drop('is_risk',axis=1)

feas = feas.merge(fea,on='rowkey',how='left')
...
#df_train = feas.merge(feas3,on='rowkey')
