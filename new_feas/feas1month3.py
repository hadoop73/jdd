...
def baseline_new(idx):
    ...
    start_date = str(t - pd.Timedelta(days=60))                     # 往前看 2 个月
    ...
    for ti in [5*60,30*60,60*60]:
        tmp = trade_data[(trade_data['trade_stamp'] >= timestamp - ti)]         # 往前看 5min/30min/1h
    ...
    for i in [0, 1]:
        try:
            res['time_diff{0}'.format(i + 1)] = timestamp - login_data.loc[i]['timestamp']                  # 距离上一次的登陆时间间隔
            ...
            res['ip_cnt{0}'.format(i + 1)] = None
            res['device_cnt{0}'.format(i + 1)] = None
            res['ip_device_cnt{0}'.format(i + 1)] = None
            res['ip_cnt_trade_rate{0}'.format(i + 1)] = None
            res['device_cnt_trade_rate{0}'.format(i + 1)] = None
            res['ip_device_cnt_trade_rate{0}'.format(i + 1)] = None
...
dtt = t_trade[t_trade['time']>='2015-03-01 00:00:00']    # 只用 4,5,6,7 做特征进行 train,valiade,test
t_trade_list = dtt.index.tolist()
del dtt

#baseline_new(t_trade_list[1])
last_f = '../datas/feas_new_month3'
