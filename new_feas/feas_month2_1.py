    ...
    try:
        ...
        for ci in [('id', 'ip'), ('id', 'device'), ('id', 'type'),
            ...
            for si, ti in zip(sis, tis):
                ...
                for tpi in [1, 2, 3]:
                    res[ci + 'type_{0}_{1}cnt'.format(ti, tpi)] = None

                for logi in [1, 2, 10, 11]:
                    res[ci + 'log_{0}_{1}cnt'.format(ti, logi)] = None

                for logi in [-2, -1, 1, 6, 31]:
                    res[ci + 'result_{0}_{1}cnt'.format(ti, logi)] = None

                for ii in ['id', 'ip', 'device', 'type', 'city']:
                    # TODO  ip,city 重复
                    if ii != ci:
                        res[ci + "_" + ii + "{0}".format(ti)] = None  # ip 之前登陆的 id 数
