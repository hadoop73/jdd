...
def baseline_new(idx):
    ...
    try:
        res['ip12_cnt'] = login_data.loc[:1]['ip'].unique().size            # 最近登陆两次的 ip 是否相等
        res['device12_cnt'] = login_data.loc[:1]['device'].unique().size    # 最近登陆两次的 device 是否相等
    except:
        res['ip12_cnt'] = None
        res['device12_cnt'] = None

    # TODO 改 max 为 sum
    res['ip_device12_sum'] = res['ip12_cnt'] + res['device12_cnt']
