...
feas = pd.read_csv('../datas/feasb1_month3')
df_train = pd.concat([feas,df_train])

#feas = feas.drop(['is_risk'],axis=1)
#df_train = df_train.merge(feas,on='rowkey',how='left')

# feas = pd.read_csv('../datas/all_3_7')
...
#         for tpi in [1, 2, 3]:
#            dlist.append(ci + 'type_{0}_{1}cnt'.format(i, tpi))
#
#         for logi in [1, 2, 10, 11]:
#             dlist.append(ci + 'log_{0}_{1}cnt'.format(i, logi))
#
#         for logi in [-2, -1, 1, 6, 31]:
#             dlist.append(ci + 'result_{0}_{1}cnt'.format(i, logi))
#     for ci in [('id', 'ip'), ('id', 'device'), ('id', 'type'),
#                ('ip', 'device'), ('ip', 'type'), ('id', 'city')]:
#         cis = ci[0] + ci[1]
#         for tpi in [1, 2, 3]:
#             dlist.append(cis + 'type_{0}_{1}cnt'.format(i, tpi))
#
#         for logi in [1, 2, 10, 11]:
#             dlist.append(cis + 'log_{0}_{1}cnt'.format(i, logi))
#
#         for logi in [-2, -1, 1, 6, 31]:
#             dlist.append(cis + 'result_{0}_{1}cnt'.format(i, logi))
#
...
#
# dlist += ['time','is_risk','id','hour']
# print(acl)
# #feas = feas[acl]
#
# feas = feas.drop(dlist,axis=1)
